...
@celery_app.task(
    name="convert_stock_quarterly_result_data",
)
def convert_stock_quarterly_result_data():
    """
    Background task to store NSE company data
    """
    run_async_task(convert_stock_quarterly_result_data_async())


@celery_app.task(
    name="fetch_stock_quarterly_result_standalone_data",
)
def fetch_stock_quarterly_result_standalone_data():
    """
    Background task to store NSE company data
    """
...
@celery_app.task(
    name="fetch_daily_gross_deliverables_bse_stock_information",
)
def fetch_gross_deliverables_nse_bse_stock_information():
...
@celery_app.task(
    name="fetch_daily_short_selling_data",
)
def fetch_daily_short_selling_data():
    run_async_task(fetch_daily_short_selling_async())


@celery_app.task(
    name="fetch_daily_bulk_deal_data",
)
def fetch_daily_bulk_deal_data():
    run_async_task(fetch_daily_bulk_deal_async())

@celery_app.task(
    name="fetch_daily_block_deal_data",
)
def fetch_daily_block_deal_data():
    run_async_task(fetch_daily_block_deal_async())